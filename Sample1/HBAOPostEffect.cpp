    ...
    : PostEffect("HBAO.fx", "HBAO_PS")
{
    ...
    GFXDEVICE->CreateSamplerState(&sampDesc, &mSamplerPointWrap);
...
void HBAOPostEffect::updateConstants()
{
    // setup constants
    {
        // params
        ...
        auto rtSize = GFX->getRenderTargetSize();

        csBuffer.FullResolution[0] = rtSize.x;
        csBuffer.FullResolution[1] = rtSize.y; 
        csBuffer.InvFullResolution[0] = 1.0 / rtSize.x;
        csBuffer.InvFullResolution[1] = 1.0 / rtSize.y;
        csBuffer.AOResolution[0] = rtSize.x;
        csBuffer.AOResolution[1] = rtSize.y;
        csBuffer.InvAOResolution[0] = 1.0 / rtSize.x;
        csBuffer.InvAOResolution[1] = 1.0 / rtSize.y;
    
        csBuffer.FocalLen[0] = 1.0f / tanf(XM_PIDIV4 * 0.5f) * (rtSize.y / rtSize.x);
        csBuffer.FocalLen[1] = 1.0f / tanf(XM_PIDIV4 * 0.5f);
        csBuffer.InvFocalLen[0] = 1.0f / csBuffer.FocalLen[0];
        csBuffer.InvFocalLen[1] = 1.0f / csBuffer.FocalLen[1];
        csBuffer.UVToViewA[0] = 2.0f * csBuffer.InvFocalLen[0];
        csBuffer.UVToViewA[1] = -2.0f * csBuffer.InvFocalLen[1];
        csBuffer.UVToViewB[0] = -1.0f * csBuffer.InvFocalLen[0];
        csBuffer.UVToViewB[1] = 1.0f * csBuffer.InvFocalLen[1];

        csBuffer.R = mRadiusScale * mSceneScale;
        csBuffer.R2 = csBuffer.R * csBuffer.R;
        csBuffer.NegInvR2 = -1.0f / csBuffer.R2;
        csBuffer.MaxRadiusPixels = 0.1f * min(csBuffer.FullResolution[0], csBuffer.FullResolution[1]);;

        csBuffer.AngleBias = mAngleBias * XM_PI / 180.0;
        csBuffer.TanAngleBias = tanf(csBuffer.AngleBias);
        csBuffer.PowExponent = mPowExponent;
        csBuffer.Strength = mStrength;

        csBuffer.BlurDepthThreshold = 2.0f * SQRT_LN2 * (mSceneScale / mBlurSharpness);
        csBuffer.BlurDepthThreshold = 2.0f * SQRT_LN2 * (mSceneScale / mBlurSharpness);
        float BlurSigma = (mBlurRadius + 1.0f) * 0.5f;
        csBuffer.BlurFalloff = INV_LN2 / (2.0f*BlurSigma*BlurSigma);
        csBuffer.LinA = 1.0f / zfar - 1.0f / znear;
        csBuffer.LinB = 1.0f / znear;

        csBuffer.nearFar[0] = znear;
        csBuffer.nearFar[1] = zfar;

        GFXCONTEXT->UpdateSubresource(hwBuffer, 0, NULL, &csBuffer, 0, 0);
    ...
    { 
        rt->getDepthStencilSRV(),
        mRandomTextureSRV
    };
    GFXCONTEXT->PSSetShaderResources(0, 2, srviews);
    ID3D11SamplerState* sstates[] = { mSamplerPointClamp, mSamplerPointWrap } ;
    GFXCONTEXT->PSSetSamplers(0, 2, sstates);
}


void HBAOPostEffect::initBuffers()
{
    HRESULT hr;
    ...
    bd.CPUAccessFlags = 0;
    bd.ByteWidth = sizeof(HBAOConstBuffer);
...
void HBAOPostEffect::createRandomTexture()
{
    ...
    INT16 *data = new INT16[4*4 * 4];
    for (UINT i = 0; i < 4*4 * 4; i += 4)
    {
        ...
        float angle = 2.0f * XM_PI * r1 / NUM_DIRECTIONS;
    ...
    srd.pSysMem = data;
    srd.SysMemPitch = 4 * 4 * sizeof(INT16);
    srd.SysMemSlicePitch = 0;

    SAFE_RELEASE(mRandomTexture);
    HRESULT hr = GFXDEVICE->CreateTexture2D(&desc, &srd, &mRandomTexture);
    if (FAILED(hr))
    {
        FUtil::Log("HBAO: Cannot create random texture.");
        return;
    }

    SAFE_RELEASE(mRandomTextureSRV);
    hr = GFXDEVICE->CreateShaderResourceView(mRandomTexture, NULL, &mRandomTextureSRV);
    if (FAILED(hr))
    {
        FUtil::Log("HBAO: Cannot create random texture SRV.");
        return;
    }
