...
// With small tampering by Damon Wall. 
...
// Must match the ROW_TILE_W value from HBAO_DX11_LIB.cpp
...
Texture2D<float> tLinDepth          : register(t1);

RWTexture2D<float2> uOutputBuffer   : register(u0);


#define SHARED_MEM_SIZE (KERNEL_RADIUS + ROW_TILE_W + KERNEL_RADIUS)
...
//----------------------------------------------------------------------------------
// Approximate horizontal cross-bilateral filter
//----------------------------------------------------------------------------------
void BlurX_CS(uint3 threadIdx, uint3 blockIdx)
{
    ...
    if (writePos < tileEndClamped)
    {
        ...
        float2 uv = ((float2(writePos,y)) + 0.5f) * InvFullResolution;
        ...
        uOutputBuffer[uint2(writePos, blockIdx.y)] = float2(ao, center_d);
    }
}
